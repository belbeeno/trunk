...
public class VolumetricAudioSource : MonoBehaviour
{
    ...
    public void MoveTowardsWhileCloseTo(Vector3 target, float maxDist, params Vector3[] verts)
    {
        ...
        float mag = (frozenTarget - minPoint).magnitude;
        minPoint = minPoint + (frozenTarget - minPoint) / mag * Mathf.Min(maxDist, mag);
        ...
        if (float.IsNaN(minPoint.x) || float.IsNaN(minPoint.y)) return;
